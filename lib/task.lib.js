...
exports.create = async (board_id, creator_id, task_type, title, description, due_date) => {
    return new Promise(async function (resolve, reject) {

        if(!board_id) return reject(new Error('No board specified'))
        if(!creator_id) return reject(new Error('No creator specified'))
        if(!task_type) return reject(new Error('No task specified'))
        else {

            let board = await board_lib.getOne(board_id);
            if(!board) return reject(new Error('Invalid board id'));

            counter_code = board.counter_appendice + '-';
            counter_code += await board_lib.incrementCounter(board._id);
...
exports.getAllBoard = async (board_id) => {
    return new Promise(async function (resolve, reject) {

        if(!board_id) return reject(new Error('No board id specified'))
        else 
            Task.find({ board_id: mongoose.Schema.Types.ObjectId(board_id)})
            .exec((e, _item) => {
                if(e) return reject(e);
                else if(_item) return resolve(_item);
                else return reject(new Error('Invalid id'))
...
exports.getAllType = async (board_id, status_type) => {
    return new Promise(async function (resolve, reject) {

        if(!board_id) return reject(new Error('No board id specified'))
        else if(!status_type) return reject(new Error('No status type id specified'))
        else 
            Task.find({ 
                board_id: mongoose.Schema.Types.ObjectId(board_id),
                status_type: mongoose.Schema.Types.ObjectId(status_type)
            }).exec((e, _item) => {
                if(e) return reject(e);
                else if(_item) return resolve(_item);
                else return reject(new Error('Invalid id'))
...
exports.getAllAssignee = async (board_id, assignee_id) => {
    return new Promise(async function (resolve, reject) {
...
exports.getAllStatus = async (board_id, status) => {
    return new Promise(async function (resolve, reject) {
...
exports.getAllCreator = async (board_id, creator_id) => {
    return new Promise(async function (resolve, reject) {
...
exports.deleteMany = async (board_id) => {
    return new Promise(async function (resolve, reject) {
        ...
        if(!board_id) return reject(new Error('No id specified'))
            ...
            Task.deleteMany({ board_id: mongoose.Schema.Types.ObjectId(board_id) }, {}, 
            (e, _item) => {
                if(e) return reject(e);
                else if(_item) {
                    //remove checklist items
